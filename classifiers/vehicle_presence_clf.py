...
from lib.detection.predictor import ParkingPredictor
from lib.detection.prediction import ParkingPrediction
...
class ResNet(nn.Module):
    ...
    def forward(self, input): # (3,64,64)
        out = self.layer0(input) # (64,16,16)
        out = self.layer1(out) #
        out = self.layer2(out)
        out = self.layer3(out)
        # out = self.layer4(out)
...
class VehiclePresenceClassifier(BaseAttributeClassifier):
    ...
    def predict(self,parking_prediction) -> str:
        ...
        output: dict of {"vehicle_presence": __vehicle_presence__ }
        """
        ...
        result['vehicle_presence'] = self._class_names[class_id]
        return result
